...
vector<int> Solution::repeatedNumber(const vector<int> &A) {
    ...
    long long int sumofNnumber=(len*(len+1))/2;
    long long int sqsum=(len*(len+1)*(2*len+1))/6;
    long long int rep=0,miss=0;
    
    for(int i=0;i<A.size();i++)
    {
        sumofNnumber=sumofNnumber-(long long int)A[i];
        sqsum=sqsum-(long long int)A[i]*(long long int)A[i]; 
    }
    
    rep=(sumofNnumber+sqsum/sumofNnumber)/2;
    miss=rep-sumofNnumber;
